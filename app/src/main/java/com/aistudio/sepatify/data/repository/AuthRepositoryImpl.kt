...
import com.aistudio.sepatify.data.remote.dto.ProfileUpdateDto
...
class AuthRepositoryImpl : AuthRepository {
    ...
    override suspend fun signUp(email: String, password: String, displayName: String): Result<Unit> {
        return try {
            ...
            if (fetchProfile() == null) {
                return Result.failure(Exception("Could not load profile after sign up."))
            }
            Result.success(Unit)
    ...
    override suspend fun setPremium(isPremium: Boolean): Result<Unit> {
        val uid = currentUserId() ?: return Result.failure(Exception("Not signed in"))
        return try {
    ...
    override suspend fun uploadAvatar(file: File): Result<String> {
        val uid = currentUserId() ?: return Result.failure(Exception("Not signed in"))
        return try {
            val path = "$uid/avatar.${file.extension.ifBlank { "png" }}"
            Supa.client.storage.from("avatars").upload(path, file.readBytes()) {
                upsert = true
            }
